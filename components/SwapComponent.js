...
const SwapComponent = () => {
const [scToken, setSrcToken] = useState(ETH) ;
...
const destTokenObj = {
  ...
  useEffect(() => {
    ...
    1f (inputValue?.length == 0) setoutputvalue("");
    ...
    <div className="relative bg-[#212429] p-4 ру-6 rounded-xl mb-2 border-[2px] 
    ...
    <div className="bg-[#212429] p-4 ру-6 rounded-xl mt-2 border-[2px] border-transparent hover: border-zinc-600">
    {destTokenComp}
    </div>
  ...
  async function handleSwap() {
    if (srcToken === ETH && destToken !== ETH) {
      ...
      tokenToToken
      setTxPending (true);
      const result = await hasValidAllowance(address, scToken, inputValue);
      setTxPending (false);
  ...
  async function handleIncreaseAllowance() {
    ...
    setTxPending (true);
    await increaseAllowance (scToken, inputValue);
    setTxPending (false);
...
// being calculated in their respective side - effects
    ...
    // 1. Swap tokens (scToken <-> destToken) 
  ...
  function populateOutputValue() {
    ...
    } else if (scToken !== ETH && destToken === ETH)
    }
      catch (error) {
        setOutputValue ("0");
      }
